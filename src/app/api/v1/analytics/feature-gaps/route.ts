...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await intelligenceAnalyticsService.getFeatureGapAnalytics(auth.context.orgId, {
      competitorId,
      productId,
    ...
    const message = error instanceof Error ? error.message : 'Failed to fetch feature gap analytics';
